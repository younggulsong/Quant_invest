...
FILE = 'quantking201105.xlsx'
FILEdate = FILE[9:15]
종목수 = 20
시가총액하위 = 10
FILENAME = "210308_가치(분기per), 모멘텀o 선별 소형주{시가총액하위} 퍼이하 부채비율100, {종목수} 개 from {FILEdate}".format(FILEdate=FILEdate,종목수 = 종목수, 시가총액하위=시가총액하위) # per이나 ev나 크게 상관없는것으로 보임..
...
sum_rank = ( ev*EV_rank+가치*sum_가치_rank + 모멘텀*sum_모멘텀_rank+gpa*GPA_rank).rank(axis =0)
sum_rank = sum_rank.sort_values()

data.insert(len(data.columns),"EV_RANK",EV_rank)
data.insert(len(data.columns),"시총EBITDA_RANK",시총EBITDA_rank)
...
month_data = month_naver_fromto(stocklist,1,10)
