...
FILE = 'quantking181026.xlsx'
FILENAME = "200823_가치(per포함), 모멘텀x 선별 소형주20퍼이하 부채비율100, 20개 from 191022" # per이나 ev나 크게 상관없는것으로 보임..
FILE_SAVE = "{FILENAME}_포트.xlsx".format(FILENAME=FILENAME)
FILE_backtest = "{FILENAME}_backtest.xlsx".format(FILENAME=FILENAME)
ev=0;가치=1;모멘텀=0;gpa=0
...
stocklist = data.head(1).index.tolist()
for i in range(0,len(stocklist)):
    stocklist[i]=stocklist[i].replace("A","")

#data11 = 변동성_naver(stocklist,240,30)
month_data = month_naver_fromto(stocklist,1,22)
