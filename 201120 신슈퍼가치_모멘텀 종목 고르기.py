...
FILE = 'quantking210709.xlsx'   #181026, 191022등으로 back test 가능
FILEdate = FILE[9:15]
종목수 = 20
시가총액하위 = 18 #시가총액 하위 퍼센또
FILENAME = "210427_신슈퍼가치, 모멘텀1년o 선별 소형주{시가총액하위} 퍼이하, 차입금비율200, {종목수} 개 from {FILEdate}".format(FILEdate=FILEdate,종목수 = 종목수,시가총액하위=시가총액하위) # per이나 ev나 크게 상관없는것으로 보임..
...
sum_rank = (가치*(PER_rank+PBR_rank+PSR_rank+PCR_rank+POR_rank+PFCR_rank+EV_rank)+모멘텀*(모멘텀_1개월_rank+모멘텀_3개월_rank+모멘텀_6개월_rank+모멘텀_9개월_rank+모멘텀_1년_rank)).rank(axis =0)
...
from quant_functions import *
month_data = month_naver_fromto_better(stocklist,'2021-04-10','2021-07-10')
