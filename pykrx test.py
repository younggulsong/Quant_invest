...
df = stock.get_market_ohlcv_by_date("20200104", "20220104", "233740") #kosdaq 레버리지
df.columns = ['open','high','low','close','volume']

data = pd.DataFrame(columns = ['upper','lower','누적수익률'])
for upper in range(2,30,2):
    for lower in range(2,30,2):
        ...
        print(new_data)
data.to_excel('220108 21년_22년kosdaq 레버리지 PC 수익률쏵.xlsx')